...
def get_frames(subdirectory, video, odir):
    ...
    if subdirectory == "FinalTest":
        ...
        subprocess.run(f"ffmpeg -i {video} -vf fps=1 "
...
def main(data_dir, label_dir, out_dir):
    ...
    subdirectories = ["FinalTest", "Train", "Validation", "FinalTest"]
    for subdirectory in subdirectories:
        ...
        DataTest = pd.read_csv(label_path)
        #label = pd.read_csv(label_path.__str__())
        ...
        with ProgressBar(max_value=len(list(sdir.glob("*/*/*")))) as bar:
            for i, video in enumerate(sdir.glob("*/*/*")):
                if DataTest['ClipID'].str.contains(video.parts[-1]).any():
